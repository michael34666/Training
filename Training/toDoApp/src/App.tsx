...
const App = () => {
  ...
  const [toDoList, setTodoList] = useState(() => {
    const toDoString = localStorage.getItem(LOCAL_STORAGE);
  ...
  const handleAdd = () => {
    ...
    setTodoList((toDoList: ToDoProps[]) => {
      return [
        ...toDoList,
        { index: toDoList.length, task: newTodo, completed: false },
      ];
    });
    setNewTodo("");
  };
  ...
  const handleDeleteTODO = (index: number) => {
    setTodoList((toDoList: ToDoProps[]) =>
      toDoList.filter((task) => task.index !== index)
    );
  };
  ...
  return (
    <>
      <h1>To Do App</h1>
      <div className="white">
        You have {howmanyneedTODO()} tasks that are open
      </div>
      <br></br>
          ...
          <ul>
            {toDoList.map((task: ToDoProps) => (
              <li key={task.index}>
                ...
                <br></br>
                <button onClick={() => handleDeleteTODO(task.index)}>
                  Delete Task
                </button>
